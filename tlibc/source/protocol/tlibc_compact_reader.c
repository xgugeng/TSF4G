...
static tlibc_error_code_t tlibc_compact_varint16_decode(const char *buff_ptr, uint32_t *buff_size, uint16_t *result)
{
	...
	*result = (b & 0x7F);
	...
	b = *(const uint8_t*)(buff_ptr + 1);
	*result |= (uint16_t)((b & 0x7F) << 7);
	if (!(b & 0x80))
	{
		*buff_size = 2;
		goto done;
	...
	b = *(const uint8_t*)(buff_ptr + 2);
	*result |= (uint16_t)((b & 0x7F) << 14);
	if (!(b & 0x80))
	{
		*buff_size = 3;
		goto done;
...
static tlibc_error_code_t tlibc_compact_varint32_decode(const char *buff_ptr, uint32_t *buff_size, uint32_t *result)
{
	...
	*result |= (uint32_t)((b & 0x7F) << 14);
	...
	b = *(const uint8_t*)(buff_ptr + 3);
	*result |= (uint32_t)((b & 0x7F) << 21);
	if (!(b & 0x80))
	{
		*buff_size = 4;
		goto done;
	...
	b = *(const uint8_t*)(buff_ptr + 4);
	*result |= (uint32_t)((b & 0x7F) << 28);
	if (!(b & 0x80))
	{
		*buff_size = 5;
		goto done;
...
static tlibc_error_code_t tlibc_compact_varint64_decode(const char *buff_ptr, uint32_t *buff_size, uint64_t *result)
{
	...
	b = *(const uint8_t*)(buff_ptr + 5);
	par1 |= (uint32_t)((b & 0x7F) << 7);
	if (!(b & 0x80))
	{
		*buff_size = 6;
		goto done;
	...
	b = *(const uint8_t*)(buff_ptr + 6);
	par1 |= (uint32_t)((b & 0x7F) << 14);
	if (!(b & 0x80))
	{
		*buff_size = 7;
		goto done;
	...
	b = *(const uint8_t*)(buff_ptr + 7);
	par1 |= (uint32_t)((b & 0x7F) << 21);
	if (!(b & 0x80))
	{
		*buff_size = 8;
		goto done;
	...
	b = *(const uint8_t*)(buff_ptr + 9);
	par2 |= (uint32_t)((b & 0x7F) << 7);
	if (!(b & 0x80))
	{
		*buff_size = 10;
		goto done;
...
tlibc_error_code_t tlibc_compact_read_int8(tlibc_abstract_reader_t *super, int8_t *val)
{
	...
	*val = *(const int8_t*)COMPACT_READER_PTR(self);
...
tlibc_error_code_t tlibc_compact_read_uint8(tlibc_abstract_reader_t *super, uint8_t *val)
{
	...
	*val = *(const uint8_t*)COMPACT_READER_PTR(self);
...
tlibc_error_code_t tlibc_compact_read_char(tlibc_abstract_reader_t *super, char *val)
{
	...
	*val = *(const char*)COMPACT_READER_PTR(self);
...
tlibc_error_code_t tlibc_compact_read_double(tlibc_abstract_reader_t *super, double *val)
{
	...
	*val = *(const double*)COMPACT_READER_PTR(self);
